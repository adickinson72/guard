"""Thanos Helm updater for Flux HelmRelease configurations."""
...
class ThanosHelmUpdater(ConfigUpdater):
    """Config updater for Thanos Flux HelmRelease files.

    This updater knows how to update Thanos version in Flux HelmRelease
    YAML configuration files (bitnami/thanos chart).
    """

    @property
    def name(self) -> str:
        """Get updater name."""
        return "thanos_helm_updater"
    ...
    async def update_version(
        ...
        """Update Thanos version in HelmRelease file.

        Args:
            file_path: Path to HelmRelease YAML file
            target_version: Thanos version to set
    ...
    async def get_current_version(self, file_path: Path) -> str:
        """Get current Thanos version from HelmRelease file.
    ...
    async def supports_file(self, file_path: Path) -> bool:
        ...
        try:
            ...
            # Check if it's a HelmRelease for Thanos
            is_helmrelease = config.get("kind") == "HelmRelease"

            # Optionally check if it's Thanos-specific
            chart_name = config.get("spec", {}).get("chart", {}).get("spec", {}).get("chart", "")
            is_thanos = "thanos" in chart_name.lower()

            return is_helmrelease and is_thanos
